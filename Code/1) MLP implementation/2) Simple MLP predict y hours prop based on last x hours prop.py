...
for i in range(len(dataset1_train) - rec - pred + 1):
    X_train.append(dataset1_train[i:i+rec, 9:10].reshape(1, -1))
...
for i in range(len(dataset2_test) - rec - pred + 1):
    X_test.append(dataset2_test[i:i+rec, 9:10].reshape(1, -1))
...
model.add(Dense(1024, activation='linear', input_shape=(input_neurons,)))
model.add(Dropout(0.25))
for i in range (1,6):
  model.add(Dense(2048, activation=LeakyReLU(alpha=0.5)))
  model.add(Dropout(0.25))
...
# Training the model
model.fit(X_train, y_train, epochs=500, batch_size=8, validation_data=(X_test, y_test), shuffle=True)
model.fit(X_train, y_train, epochs=500, batch_size=8, validation_data=(X_test, y_test), callbacks=[lr_callback], shuffle=True)
...
save_path_test =  '/content/drive/MyDrive/Colab Notebooks/Results/Simple_Prop_X_test_results_comparison.csv'
save_path_train = '/content/drive/MyDrive/Colab Notebooks/Results/Simple_Prop_X_train_results_comparison.csv'
